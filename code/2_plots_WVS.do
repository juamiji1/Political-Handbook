...
*
*-------------------------------------------------------------------------------
use "${idata}\wvs7_gdp_country_year_lvl.dta", clear 

*Predict gdp with FEs
reg ln_gdp i.year i.regionwb
predict gdp_u, resid
...
foreach var of global trust_inv {
	...
	reg `var' i.year i.regionwb
	predict `var'_u, resid

	local varlabel : variable label `var'
	two (scatter `var'_u gdp_u) (lowess `var'_u gdp_u) (lfit `var'_u gdp_u), ytitle("`varlabel'", size(medsmall)) legend(off)
	gr export "${plots}/scatter_`var'_u.pdf", as(pdf) replace
}
